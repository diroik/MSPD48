...
static void _modbus_tcp_close(modbus_t *ctx)
{
    if (ctx->s != -1)
    {
        if (ctx->debug) {
        	modbus_tcp_t *ctx_tcp = (modbus_tcp_t *)ctx->backend_data;
        	printf("tcp_close conection %s:%d\n", ctx_tcp->ip, ctx_tcp->port);
        }
...
modbus_t* modbus_new_tcp(const char *ip, int port)
{
    ...
    if (ip != NULL) {
        ...
        memset(ctx_tcp->ip, 0, dest_size);
        ret_size = strlen(strncpy(ctx_tcp->ip, ip, dest_size));//TODO test!!!!
        if (ret_size == 0) {
...
modbus_t* modbus_new_tcp_pi(const char *node, const char *service)
{
    ...
    if (node == NULL) {
        ...
        ret_size =  strlen(strncpy(ctx_tcp_pi->node, node, dest_size));//TODO test!!!
        if (ret_size == 0) {
    ...
    if (service != NULL) {
        ...
        ret_size =  strlen(strncpy(ctx_tcp_pi->service, service, dest_size));//TODO test!!!
    } else {
