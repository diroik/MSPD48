...
static void _sleep_response_timeout(modbus_t *ctx)
{
    ...
    //sleep(ctx->response_timeout.tv_sec);
    //usleep(ctx->response_timeout.tv_usec);
...
int _modbus_receive_msg(modbus_t *ctx, uint8_t *msg, msg_type_t msg_type)
{
    ...
    while (length_to_read != 0)
    {
        ...
        //Log::DEBUG( "[ModBus] _modbus_receive_msg: length_to_read="+toString(length_to_read) + " rc="+toString(rc) + " p_tv.tv_sec="+toString((int)p_tv->tv_sec) +" ctx->debug="+toString(ctx->debug));
...
static int read_registers(modbus_t *ctx, int function, int addr, int nb,
                          uint16_t *dest)
{
	//Log::DEBUG( "[ModBus] read_registers");
    ...
    //////////////////////////
    		 Byte bf[2] {0,0};
    		  ...
    		  {
    			  ByteToHex(bf, req[i]);
    			  bytes += "0x"+string((char*)bf, 2)+" ";
    		  }
    		//Log::DEBUG( "[ModBus] send_msg: [" + bytes +"] len="+toString(req_length));
    	    //////////////////////////
...
static int write_single(modbus_t *ctx, int function, int addr, int value)
{
    ...
    /*
    	//////////////////////////
		 ...
		 Log::DEBUG( "write_single [" + recv +"] len="+toString(req_length)+" value="+toString(value));
